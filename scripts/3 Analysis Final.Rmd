...
var_names_2 <- as_labeller(
     c(`AvgDBH_small` = "Average Small DBH", `AvgDBH_med` = "Average Medium DBH", `AvgDBH_large` = "Average Large DBH",  `Evar` = "Evenness",`pct_2.97` = "Ideal Pits", `PercGood` = "% Good", `PercPoor` = "% Poor", `percent_filled` = "% Filled", `rare_rich` = "Richness", `PercDead` ="Dead Stock"))
...
Dependent |> 
  select(
    ...
    , 'Sites Above Target Pit Size (%)' = pct_2.97
...
#15 Target Pit Size
```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
pct_2.97_r <- p_filtered_0.05 %>% 
  filter(Variable1 == "pct_2.97" | Variable2 == "pct_2.97")

view(pct_2.97_r)

# p<0.001
pct_2.97_r2 <- p_filtered_0.001 %>% 
  filter(Variable1 == "pct_2.97" | Variable2 == "pct_2.97")
