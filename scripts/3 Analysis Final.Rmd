...
totpits_r <- p_filtered_0.05 %>%
  ...
  mutate(padj=p.adjust(PValue, method='BH', n=99))
...
#%poor vs % black
D <- ggplot(Combined, aes(x = PopDensity, y = spave))+
  geom_point(color = "lightblue", size=2.5)+
  labs(x = "Population density", y = "Beta diversity") + 
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=12))

#%target pit size vs. percent impervious
E <- ggplot(Combined, aes(x = PercentImp, y = Perc2.97))+
  geom_point(color = 'red', size=2.5)+
  labs(x = 'Impervious surface (%)', y = "Sites above target\npit size (%)") + 
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=12))
