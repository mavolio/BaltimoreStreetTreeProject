...
Combined |>
  ...
  # ggsave(
  #   filename = paste0('figures/Figure_Table_3_corellation_', Sys.Date(), '.png'))
...
C <- ggplot(Combined, aes(x = avg_temp, y = rich_tottree))+
  ...
  labs(x = "Air Temperature (°C)", y = "Relative Species Richness") + 
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=12))
...
F <- ggplot(Combined, aes(x = avg_temp, y = AvgDBH_large))+
  ...
  labs(x = "Air Temperature (°C)", y = "Average DBH of Large Species (in.)") + 
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=12))
```
...
ggsave(paste0('figures/Figure_2_scatterplot_grid_', Sys.Date(), ".jpg"),width=180, height=200, unit="mm", plot=Fig_grid, dpi=300 )
```
