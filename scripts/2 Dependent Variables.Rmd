...
# how many have a length >20 OR width > 20 AND fail to meet pit area threshold of 9m (so 3 units?)

site_status |> 
  filter(tree_present == 1) |> 
  mutate(  length = as.numeric(ifelse(SPCELENGTH == '>20', 21, SPCELENGTH)) 
         , width  = as.numeric(ifelse(SPACEWIDTH == '>20', 21, SPACEWIDTH))
         , length_SI = length / 3.281 # *0.3048 #convert to SI
         , width_SI  = width / 3.281 # *0.3048 #convert to SI
         , pit_area  = length_SI*width_SI
         , reviewer_query = ifelse(
              # (length > 20 | width > 20) & pit_area < 9, 'yes', 'no' # IS this right?
                (length > 20 | width > 20) & pit_area < 3, 'yes', 'no' # IS this right?
                )
        ) |> 
  janitor::tabyl(reviewer_query) 
...
pit_size <- 
  ...
  mutate(  length_SI = length / 3.281 # *0.3048 #convert to SI
         , width_SI  = width / 3.281 # *0.3048 #convert to SI
         , pit_area  = length_SI*width_SI
         ) %>% 
...
#   tabyl(size_threshhold)

large_pit <- 
  pit_size %>% 
  mutate(size_threshhold = ifelse(pit_area >= 2.97, "greater", "less" )) %>% # 2.97 or 9?
  ...
  mutate(pct_2.97 = (greater / T1)*100) %>% 
  select(CSA2020, pct_2.97)

large_pit |> ggplot(aes(pct_2.97)) + geom_histogram() #geom_density()
...
#15 Condition and Pit Area (exploratory)
```{r}
