...
```{r include=FALSE}
site_status <- clean %>% 
  mutate(tree_present=case_when(
    Species %in% c('Vacant Site', 'Vacant Potential','Stump','Vacant Site Not Suitable','NA', "Z Add 01", "Dead") ~ 0
...
perc_filled<-site_status %>% 
  ...
  pivot_wider(id_cols=CSA2020, names_from=tree_present, values_from=n, names_prefix="T") %>% 
...
vacant_potential <- clean %>% 
  filter(Species=='Vacant Site'|Species=='Vacant Potential'|Species=='Stump'|Species=='NA'|Species=="Z Add 01"|Species=="Dead")%>% #Stumps and dead no longer considered vacant?
...
street_tree_size_classes <- no_vacant %>% 
  left_join(size_classes, by = "Species") 

#list of species with no size class in iTree data
unclassified_species <- street_tree_size_classes %>%
  filter(is.na(`Size Class`)) %>% 
  distinct(Species)

#small trees
AvgDBH_small <- street_tree_size_classes %>% 
...
#medium trees
AvgDBH_med <- street_tree_size_classes %>% 
...
#large trees
AvgDBH_large <- street_tree_size_classes %>% 
