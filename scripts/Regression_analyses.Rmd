...
Combined |> 
  select(CSA2020 : total_trees, sum_road_length_ft) |> 
  left_join(neighs |> select(CSA2020, area_km2), by = 'CSA2020') |> 
...
# 1 read in neighborhood boundaries and join to variables
```{r}
...
# TK Trini there are missing values 
  neighs |> 
  select(  CSA2020
          # dependent variables
         , rich_tot        # Richness normalized by total trees
...
mods_bi <- 
  ...
  select(  CSA2020
         ...
         , bahigher20
         , avg_temp
         # , PercentImp    # temporarily turned off due to missingness
         # , PopDensity    # temporarily turned off due to missingness
         # , road_length_m # temporarily turned off due to missingness
         ) |> 
  # pivot_longer(cols = PercBlk : road_length_m, names_to = 'ind', values_to = 'ind_value') |> # temporarily turned off due to missingness
  pivot_longer(cols = PercBlk : avg_temp, names_to = 'ind', values_to = 'ind_value') |>
  pivot_longer(cols = rich_tot : Perc2.97, names_to = 'dep', values_to = 'dep_value') |> 
