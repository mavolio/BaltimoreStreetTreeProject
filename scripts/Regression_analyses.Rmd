...
#    , AvgDBH_med      # Small Tree Average DBH no longer a thing
    ...
    , pct_2.97        # % of trees above threshold 
...
mods_bi <- 
  ...
  pivot_longer(cols = rare_rich : pct_2.97, names_to = 'dep', values_to = 'dep_value') |> 
...
mods_bi_slag <- 
  ...
  mutate(
    ...
    , pct_2.97_lag        = st_lag(pct_2.97        , nbs, wts)
  ) |> 
  st_drop_geometry() |> 
  select(CSA2020          # id
         # dep 
  ...
  pivot_longer(cols = PercBlk : vacant20, names_to = 'ind', values_to = 'ind_value') |> 
  pivot_longer(cols = rare_rich_lag : pct_2.97_lag, names_to = 'lag', values_to = 'lag_value') |> 
  pivot_longer(cols = rare_rich : pct_2.97, names_to = 'dep', values_to = 'dep_value') |> 
...
mods_bi_slag |>
  ...
  mutate(
    p_adjust_bh = p.adjust(p.value, method = 'BH', n = n_adjust)
    , sig = ifelse(p_adjust_bh, 'not sig', 'sig')) |>
  janitor::tabyl(sig)
...
sp_lag_sigs_tbl <-
  ...
  filter(term == 'ind_value') |> 
  mutate(
    p_adjust_bh = p.adjust(p.value, method = 'BH', n = n_adjust) # 77
  ...
  select(dep, ind, `Spatial Lag Regression\nSign & Significance`)

sp_lag_sigs_tbl |> janitor::tabyl(`Spatial Lag Regression\nSign & Significance`)
...
spat_vs_corrs <- 
  ...
  filter(var_set_1 != var_set_2) |> 
   mutate(
       p_adjust_bh = p.adjust(p, method = 'BH', n = n_adjust)
     ,`Correlation\nSign & Significance` = factor(
      case_when(
...
spat_vs_corrs |> 
  janitor::tabyl(  `Spatial Lag Regression\nSign & Significance`       # ROWS 
                 , `Correlation\nSign & Significance` # COLS
                 ) |> 
...
spat_vs_corrs |> 
  ...
  count() |> 
  arrange(desc(n)) #|> 
  # write_csv(paste0('output_tables/spat_vs_corrs_bh_adjust_counts_', Sys.Date(), '.csv'))
...
spat_vs_corrs |> 
  ...
  left_join(
    var_alias_lu |> select(name, alias)
    , by = c('p1' = 'name')
    ) |> 
  left_join(
    var_alias_lu |> select(name, alias)
  ...
  select(
      'Street Tree Variables' = alias.x
    , 'Socio-Environmental Variables' = alias.y
    , `Correlation\nSign & Significance`
    , `Spatial Lag Regression\nSign & Significance`
  ) #|> 
  #writexl::write_xlsx(paste0('output_tables/Table_S1_spat_vs_corrs_bh_adjust_counts_', Sys.Date(), '.xlsx'))
...
mods_bi_slag |>
  ...
  ggplot(aes(  x
             , predicted
             , color = `Spatial Lag Regression\nSign & Significance`
             , fill = `Spatial Lag Regression\nSign & Significance`)
         ) + 
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.5) + 
  # facet_wrap(~group, scales = 'free') +
  ...
  labs(  title = 'Relationships betwen independent and dependent variables'
       ...
       , captoin = 'p values adjusted with a Benjamini & Hochberg correction'
       ) + 
  NULL
