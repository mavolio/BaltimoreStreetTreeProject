...
#16 Num Flowering
```{r}
numflor_r <- p_filtered_0.05 %>% 
  filter(Variable1 == "num_flor" | Variable2 == "num_flor")
...
#17 Num showy
```{r}
numshowy_r <- p_filtered_0.05 %>% 
  filter(Variable1 == "num_showy" | Variable2 == "num_showy")
...
B <- ggplot(Combined, aes(x = avg_temp, y = rich_tot))+
  geom_point(color = "#ffd919", size=2.5)+
  labs(x = "Temperature (C)", y = "Richness") + 
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=15))
#richness vs % black
C <- ggplot(Combined, aes(x = PercBlk, y = rich_tot))+
  ...
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=15))
#DBH vs temp
D <- ggplot(Combined, aes(x = avg_temp, y = AvgDBH))+
  ...
  theme_bw()+
    theme(panel.grid=element_blank(),
        text=element_text(size=15))
#%poor vs % black
E <- ggplot(Combined, aes(x = PercBlk, y = PercPoor))+
...
```{r}
library(gtable)
Fig_grid <- grid.arrange(C, E, B, D, G, A, nrow=3)
