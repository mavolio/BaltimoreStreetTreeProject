...
#neighborhood demographics data frame
NBdem<- data%>%
...
cor.test(richeven$Evar,richeven$PercentWhite)
...
```{r}
neighs <- read.csv('../BaltimoreStreetTreeProject_Large_Data/neighs.csv') %>% 
  rename(Number = X357459) %>% 
  rename (Neighborhood = Beechfield)
  

#pivot column and join with NB demographics 
NBland <- neighs %>%
  pivot_wider(names_from = Tree.Canopy, values_from = Number) %>% 
  left_join(NBdem)
head(NBland)
```
...
```{r}
(test <- 
    read_csv('../BaltimoreStreetTreeProject_Large_Data/neighs.csv'
