...
#what's diff between vacant site and vacant potential
#existing trees column
```
...
#6 Total # Trees per Neighborhood - how filled is a neighborhood
```{r}
NBTotal <-no_vacant %>% 
  group_by(CSA2010) %>% 
...
common_name <- no_vacant %>% 
  ...
  unique()
# 
# n_vs_i <- #this is the file specifying whether a species is native or introduced
#   read.csv("../../BaltimoreStreetTreeProject_Large_no_vacant/year-plot-species-abundance-addnotree.csv") %>% 
...
#Trying to calculate percent native
# PercNative <- native %>% 
#   left_join(NBTotal) %>% 
#   mutate(present_native=ifelse(Nat == 'native', 1, 0)) %>% 
