...
total_sites <- vacant_potential %>% 
  ...
  select(CSA2010, all_sites, percent_filled)
...
PercSize <- DBHclass %>% 
  ...
  select(CSA2010,PercSmall, PercMedium, PercLarge)
```
...
pit_size <- clean %>% 
  ...
  select(CSA2010, pit_area)

ggplot(pit_size, aes(x=pit_area)) + geom_histogram()

#average pit size by nb
AvgPit <- pit_size %>%  
  group_by(CSA2010) %>% 
  summarise(AvgPit = mean(pit_area), .groups = 'drop')

#avg pit size only filled pits
no_vacant_pit <- no_vacant %>% 
...
condition_grade <- no_vacant %>% 
  ...
  select(CSA2010,grade) %>% 
  group_by(CSA2010)

#Condition vs pit size
cor.test(no_vacant_pit$pit_area, condition_grade$grade)

plot(no_vacant_pit$pit_area, condition_grade$grade)
```

#22 % good/poor trees
```{r}
perc_condition <- NBTotal %>% 
  left_join(condition, by = "CSA2010") %>% 
  mutate(PercGood = 100*(Good/total_trees)) %>% 
  mutate(PercPoor = 100*(Poor/total_trees)) %>% 
  select(CSA2010, PercPoor, PercGood)
```

# Small Trees Species Abundance
```{r}
small_trees <- no_vacant %>%
  filter(DBH>=0&DBH<=5) %>% 
  group_by(CSA2010)

small_species_abund <- small_trees %>%
  group_by(CSA2010, Species) %>% 
  summarize(n=length(Species)) %>%
  pivot_wider(names_from = Species, values_from = n, values_fill = 0) %>% 
  left_join(NBtemperature, by = "CSA2010") %>% 
  filter(avg_temp>31.6) %>%
  arrange(desc(avg_temp)) %>% 
  select(-avg_temp)

species_long <- small_species_abund %>% 
  gather(key = "TreeSpecies", value = "Count", -CSA2010)

most_common_species <- species_long %>% 
  group_by(CSA2010) %>% 
  slice_max(order_by = Count, n=1) 

#top species for each NB
ggplot( most_common_species, aes(x = TreeSpecies, y = Count, fill = CSA2010)) +
  geom_bar(stat = "identity") +
  labs(title = "Most Common Species in Neighborhoods hotter than 31.6 Celsius") +
  theme_minimal() 

#top species for one NB
madison_filtered <- small_species_abund %>% 
  filter(CSA2010 == "Madison/East End") 

madison_long <- madison_filtered %>% 
  gather(key = "TreeSpecies", value = "Count", -CSA2010)

madison_top_species <- madison_long %>%
...
#Patterson Park
patterson_filtered <- small_species_abund %>% 
  filter(CSA2010 == "Patterson Park North & East") 

patterson_long <- patterson_filtered %>% 
  gather(key = "TreeSpecies", value = "Count", -CSA2010)

patterson_top_species <- patterson_long %>%
  slice_max(order_by = Count, n = 5)
...
oldtown_filtered <- small_species_abund %>% 
  filter(CSA2010 == "Oldtown/Middle East") 

oldtown_long <- patterson_filtered %>% 
  gather(key = "TreeSpecies", value = "Count", -CSA2010)

oldtown_top_species <- oldtown_long %>%
  slice_max(order_by = Count, n = 5)
...
```{r}
 DepVar <- total_sites %>% 
  ...
  left_join(perc_condition, by = "CSA2010") %>%
  left_join(PercSize, by = "CSA2010") 
