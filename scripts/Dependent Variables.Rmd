...
```{r include=FALSE}
combined <-
  read_csv('../../BaltimoreStreetTreeProject_Large_Data/street_trees_with_neigh_attributes_2023-07-24.csv')
```
#4 Clean combined data
```{r}
clean <- combined %>% 
  select(-ID,-OnStr, -OBSERV2, -OBSERV3, -LOC_TYPE, -HARD_SCAPE, -PARCELID, -COLLECTOR, -Inv_Time, -INSPECT_DT, -Inspect_TM, -Notes, -Link, -UniqueID, -UTILITIES, -Community, -Neigh) %>% 
...
Total_st <- NBTotal %>% 
  left_join(NBRoads) %>% 
  mutate(ratio = NBTotal$total_trees/NBRoads$sum_road_length_ft)
