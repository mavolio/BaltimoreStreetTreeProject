...
PercentLand <- LandCover %>% 
  ...
  select(CSA2010, Imperv, PercentImp, Pervious, PercPervious)
```
...
PercentCanopy <- LandCover %>% 
  ...
  select(CSA2010, PercCanopy)
```

NO #11 total road length (not including highways)
```{r}
...
# 
# roads %>% filter(MUNICIPALI == 'N/A') %>% View()
# 
# roads_to_exclude <- 
#   c(PUT JONES FALLS, ETC in here)
# 
# road <- 
#   roads %>% 
#   filter(road %nin% roads_to_exclude)
