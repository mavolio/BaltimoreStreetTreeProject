...
PercentAsian <-  
  ...
  select(OBJECTID, CSA2010, pasi20) %>% 
...
PercentWhite <- 
  ...
  select(OBJECTID, CSA2020, pwhite20) %>% 
...
Vacant <- 
  ...
  select(OBJECTID, CSA2020, vacant20)
```
...
Income <- 
  ...
  select(OBJECTID, CSA2020, mhhi20)
```
...
PercentLand <- LandCover %>% 
  ...
  mutate(Pervious = Water + `Tree Canopy`+ 
 #          `Scrub\Shrub` #to fix = Error: '\S' is an unrecognized escape in character string 
         + Herbaceous + Barren) %>% 
...
```{r}
IndVar <- PercentAsian %>% 
  left_join(PercentLatino) %>% 
  left_join(PercentBlk) %>%
  left_join(PercentWhite) %>% 
  left_join(TotalPop) %>% 
#  left_join(Income) %>%
  left_join(Poverty) %>% 
  left_join(Edu) %>% 
  ...
  left_join(PercentCanopy) %>% 
  left_join(PercentLand)
